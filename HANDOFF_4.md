...
Last updated: Session 11 complete — #120 (search input logged), #121 (week selector logged), roadmap updated. No code changes this session. Next: #105 role gating UAT.
...
| Table | Policy |
|---|---|
| staffing_allocations | tenant_id = jwt tenant_id |
| employees | tenant_id = jwt tenant_id |
| projects | tenant_id = jwt tenant_id |
| supply | tenant_id = jwt tenant_id |
| demand | tenant_id = jwt tenant_id |
| rates | tenant_id = jwt tenant_id |
| non_billable_projects | tenant_id = jwt tenant_id |
| users | tenant_id = jwt tenant_id |
| tenants | id = jwt tenant_id |

RLS uses: auth.jwt() -> 'app_metadata' ->> 'tenant_id'
All read endpoints use serviceClient (bypasses RLS) — do NOT revert to user JWT for reads.
...
4. #120 — Wire employee/project search input (client-side keyup filter, no server round-trip)
   - Confirm overlap with #110 before starting — may be same issue
5. #121 — Wire week selector dropdown on heatmap
   - Confirm overlap with #111 before starting — may be same issue
6. #110 — Wire header search bar (close as duplicate of #120 if confirmed same)
7. #111 — Wire date range selector on heatmap (close as duplicate of #121 if confirmed same)
...
Read HANDOFF_4.md in the repo root and give me a 5-bullet summary of current state, then confirm the first issue to tackle.

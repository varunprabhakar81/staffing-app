...
Last updated: Session 12 complete — #105 Role Gating UAT done, critical save bug fixed (serviceClient), stale roles cleaned up, #122 logged. Next: #109 isBillable fix.
...
Root cause: POST /api/save-staffing and POST /api/supply/update were calling
readStaffingData(req.session.token) and upsertAssignment with the user JWT.
req.session.token was null — so RLS blocked all Supabase queries, returning 0
supply rows. Saves appeared to succeed (200) but nothing was written.

Fix: All write routes now use serviceClient (same as read routes):
- readStaffingData(null, serviceClient) in save-staffing and supply/update
- upsertAssignment passes serviceClient internally
- Do NOT revert to req.session.token for any data operations

Also added 1500ms delay before loadDashboard() post-save to allow Supabase
propagation before re-render.
...
* pm_test@test.com (or similar) — project_manager
...
2. #104 — Settings tab styling inconsistencies
3. #120 — Wire employee/project search input (client-side keyup filter)
   - Confirm overlap with #110 before starting — may be same issue
4. #121 — Wire week selector dropdown on heatmap
   - Confirm overlap with #111 before starting — may be same issue
5. #122 — Executive overview projects missing (low priority)
6. #110 — Wire header search bar (close as duplicate of #120 if confirmed)
7. #111 — Wire date range selector (close as duplicate of #121 if confirmed)
...
* #120 and #110 may be duplicates — confirm before working on either
* #121 and #111 may be duplicates — confirm before working on either
...
Read HANDOFF_5.md in the repo root and give me a 5-bullet summary of current state, then confirm the first issue to tackle.

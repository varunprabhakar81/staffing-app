...
| 4 | #193 Ph1 | Project heatmap — read-only with consultant cross-link | Medium |
| 5 | #210 Ph3 | Command palette: Contextual page filters | Medium |
| 6 | #238 | Command palette: Consultant result → Resource Allocation row | High |
| 7 | #237 | Command palette: Search open needs | Medium |
| 8 | #208 | Skill set categories rethink — structured grouping | Medium-Large |
| 9 | #231 | Testing portal: retest requires double-press to pass | Medium |
| 10 | #233 | Testing portal: optional notes on passed test cases | Medium |
| 11 | #230 | Closed needs section (met + abandoned) on Open Needs tab | Medium |
| 12 | #183 | Contextual tooltips | Small |
| 13 | #184 | Admin getting-started checklist | Medium |
| 14 | #182 | In-app onboarding tour | Medium |
| 15 | #220 | Bulk assign UI — row-select mechanism | Medium |
| 16 | #235 | Overview: Reintroduce project utilization KPI (post-Projects page) | Low |
...
| 20 | #234 | Testing portal: progress bar filter | Low |
| 21 | #232 | Audit trail for cancelled invites/closed needs | Low |
| 22 | #236 | Testing portal: visual screenshots for navigation paths | Medium |
...
- Invite flow rework: Own the token (7-day TTL, single-use, no Supabase dependency)
...
- S37 backlog issues created (#229–234)
- S38 backlog issues created (#235–238)
...
- Fixed expired token issues (Tim, Shreyas, Nick stuck confirmed-but-no-password)
...
- Removed "Projects with Most Utilization" KPI
- Added "Open Needs by Urgency" panel
...
- New endpoints: POST /api/auth/validate-invite, POST /api/auth/set-password, POST /api/admin/users/:id/reset-password
...
- General feedback append logic (server-side)
- General feedback textarea always editable
- testing.html: "Heatmap" → "Resource Allocation" rename
...
| Decision | Rationale |
|----------|-----------|
| Own invite tokens (not Supabase generateLink) | Supabase tokens expire quickly, get consumed on click even when expired. Our tokens: 7-day TTL, single-use, we control lifecycle. |
...
4. #193 Ph1 — Read-only project heatmap
5. Overview "wow layer" — sparklines, trend indicators, load animations (create issue)
6. Add Change Password section to Settings for all users
7. Collect and triage pilot tester feedback

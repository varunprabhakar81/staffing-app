# Staffing Intelligence App — Session 25 Handoff
Last updated: Session 25 complete
...
* Open Needs: 2-segment donut chart (Partially Met + Unmet only) + expandable rows with AI match panel. AI recommendations use any-skill matching (allSkillSets.includes()). Create New Need button (admin + resource_manager): 2-step modal with inline project creation. Abandon button on each row closes need with reason='abandoned'. Needs auto-close when fully met via checkAndAutoCloseNeeds().
...
* table-layout: fixed; column widths redistributed for new columns
...
| V2 | In progress | 7 open — #165, #173, #170, #168, #154, #129, #162 (epic) |
| V3 | Backlog | 22 open — tenant onboarding, Finance dashboard, integrations, extended roles, and more |
...
## Issues Completed This Session (Session 25)

| Issue | Title | Notes |
|---|---|---|
| #164 | Create, edit, and close staffing needs — full Open Needs CRUD | Full need lifecycle: create (2-step modal + inline project creation), auto-close on fully met, manual abandon. Client/Project columns in table. Donut chart reduced to 2 segments. All UAT passed. |

---

## New Issues Created This Session (Session 25)

| Issue | Title | Milestone | Labels |
|---|---|---|---|
| #168 | Comprehensive multi-user UAT test suite | V2 | type:enhancement · p1 · l · area:infra |
| #169 | PM need-creation approval workflow | V3 | type:feature · p2 · m · area:open-needs |
| #170 | Synthetic test data — replace real project/need data | V2 | type:chore · p2 · m · area:infra |
| #171 | Improved date input UX across the app | V3 | type:enhancement · p3 · m · area:open-needs |
| #172 | Client data master with client→project hierarchy | V3 | type:feature · p2 · l · area:settings |
| #173 | Edit existing staffing need | V2 | type:feature · p1 · s · area:open-needs |

---

## Key Technical Decisions (Session 25)

* **closed_at TIMESTAMPTZ + closed_reason TEXT pattern** — need lifecycle uses nullable timestamp (not boolean, not status enum). closed_reason: 'met' | 'abandoned'. Clean, queryable, auditable.
* **Auto-close triggers inside /api/supply/update** via checkAndAutoCloseNeeds() — runs after every acceptMatch() write. Not a scheduled job; fires on the write that satisfies the need.
* **Inline project creation pushes into _cnProjects array** (not just DOM) — prevents date pre-fill bug when user selects a just-created project before the dropdown re-renders from server.
* **is_billable defaults to true** in createProject() — was causing NOT NULL DB violation without a default.
* **Donut chart permanently 2 segments** (Partially Met + Unmet) — Fully Met removed. Since closed_at IS NULL filters the dataset, fully_met can never appear. statusMap, legend, and applyNeedsFilter all updated to 2-element arrays.
* **GET /api/projects?status= filter** — supports filtering by project status. Create Need modal excludes Proposed projects from selection dropdown.
* **table-layout: fixed** redistribution for 9-column needs table — Client and Project added as first two columns.
...
* **resolveConsultantId, resolveProjectId, upsertAssignment, deleteAssignments, createProject(), createNeed(), closeNeed() all use serviceClient — not user JWT. RLS blocks user JWT on all write-path functions. Never revert.**
...
* closed_at + closed_reason pattern chosen for need lifecycle (not boolean, not enum)
* Auto-close fires inside /api/supply/update, not as a scheduled job
* Fully Met permanently removed from donut chart — dataset guarantee makes it unreachable
* Inline project creation must push into _cnProjects array, not only DOM
* is_billable must default to true on createProject() — NOT NULL constraint
* #163 and #156 remain closed — their scope fully absorbed into #164
* Session docs: HANDOFF + CLAUDE.md only (confirmed from Session 24)
...
V2 remaining (7 open issues, priority order):
1. **#165 — Settings nav menu** — sectioned nav (Users / Consultants). Small, closes #162 epic. ← START HERE
2. **#173 — Edit existing staffing need** — edit modal pre-populated from row, similar to Create Need Step 2
3. **#170 — Synthetic test data** — replace real project/need data with synthetic dataset; include Proposed project + varied coverage states
4. **#168 — Multi-user UAT suite** — run after #165 + #173 + #170 are done; Phase 1 (all admin) then Phase 2 (real roles)
5. **#154 — Holistic UI/UX design pass** — absorbs SB-2, SB-4, SB-7
6. **#129 — Historical staffing snapshots** — time-travel view, effort:xl
7. **#162 — Epic gate** — closes automatically when #165 closes

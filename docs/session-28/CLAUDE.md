...
- **Cache busters must be incremented on every deploy with frontend changes** — `app.js` and `styles.css` both carry `?v=N` query strings in `index.html`. Current: `app.js?v=98`, `styles.css?v=50`.
...
### V2 — Ship it (5 issues)
1. **#175 — RM can triage needs by client account** ← next

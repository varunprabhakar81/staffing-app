...
- **Recommendations engine: candidates must match level + any skill (`allSkillSets.includes`)** — Included if average available hours across demand range >= 20% of hours needed (`avgAvailable >= hoursNeeded * 0.2`). Sorted by `availableHours` desc. Badge color is informational only (green ≥100%, yellow 50–99%, coral <50%). Do not reintroduce a hard per-week qualifying gate.

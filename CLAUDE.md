...
- **Recommendations engine: two-tier matching** — Tier 1: `qualifyingWeeks > 0` (consultant has full capacity in ≥1 week). Tier 2: consultant has ≥20% of needed hours available on average across the demand range (`avgAvailable >= hoursNeeded * 0.2`). Both tiers use `allSkillSets.includes()` for skill matching. Tier 2 candidates carry `partialCandidate: true`. Tier 1 candidates sort first. Do not revert to single-tier gate.

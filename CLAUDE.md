...
- **Recommendations engine: candidates must match level + any skill (`allSkillSets.includes`)** — Included if average available hours > 0 across demand range. Sorted by `availableHours` desc. Badge color is informational only (green ≥100%, yellow 50–99%, coral <50%). Do not reintroduce a hard per-week qualifying gate or minimum availability threshold.
